...
// 🔥 FLUID SIMULATION SHADER (FIXED NAME)
export const fluidShader = `
    ...
    float ln(vec2 p, vec2 a, vec2 b) {
        return length(
            p - a - (b - a) * clamp(dot(p - a, b - a) / dot(b - a, b - a), 0., 1.)
        );
    }

    vec4 t(vec2 v, int a, int b) {
        return texture2D(uPreviousFrame, fract((v + vec2(float(a), float(b))) / ur));
    }

    vec4 t(vec2 v) {
        return texture2D(uPreviousFrame, fract(v / ur));
    }
    ...
    void main() {
        ...
        if (iFrame < 1) {
            ...
            if (iMouse.z > 0.0) {
                ...
                // original stroke push
                me.xyw += strengthFactor * falloff * vec3(m, 10.0);

                // ----------------------------
                // BLACK CIRCLE BRUSH TRAIL
                // ----------------------------
                float dist = length(U - mousePos);
                float circle = smoothstep(uBrushRadius, 0.0, dist);

                // add density (ink)
                me.z += circle * uBrushStrength * 0.5;

                // outward pressure
                me.xy += normalize(U - mousePos) * circle * 0.002;
